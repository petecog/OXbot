...
# YOU WILL NEED TO INSTALL TENSORFLOW INTO YOUR ENVIRONMENT!!!
...
# set and added to my personal set for that game.
...
import tensorflow as tf # For cool ML shiznit

# IF THE BOARD PLOTTING FUCKS UP, UNCOMMENT THIS AND RUN!
%matplotlib inline
# 
...
class Moses: # A referee that sets up and can run a game; 
    ...
    def rungame(self): # Administers the game until victory achieved; returns 0 / 1 for winning player, or -1 for a draw
        ...
        if self.players[self.game.player] == 0: move = self.askhuman() #human player
        if self.players[self.game.player] == 1: move = self.random_move() #random player
        if self.players[self.game.player] > 2: move = self.askbot(self.game.player) #otherwise ask the appropriate bot
        
        result = self.game.update(move[0], move[1]) # -1 = error, 0 = game continues, 0.5 = ends with draw, 1 = victory for active player
        
        if self.rend == 1: print(move); self.game.render() # Draw the board if required
        
        if result == 1: return self.game.player # Game has ended with a victory 
        if result == 0.5: return -1 # Game has ended with a draw
        if result == -1: print('Illegal move')
        
        return self.rungame() # If not already ended, carries on playing
...
# class Godule: # This manages the evolution process
    # return
    
# class Bot(): # A machine that takes a gamestate, and returns a set / number combination
    
#     def perceptron(x, weights, biases, keep_prob):
